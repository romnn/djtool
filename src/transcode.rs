...
pub struct Transcoder {
    stream: usize,
    input: format::context::input::Input,
    output: format::context::output::Output,
    filter: filter::Graph,
    decoder: codec::decoder::Audio,
    encoder: codec::encoder::Audio,
...
//         // If this seek was embedded in the transcoding loop, a call of lush()        // for every opened buffer after the successful seek would be advisable.
...
//         .expect("failed to find MP3 encoder")
//         .audio()?;
//     octx.format().codec(path, media::Type::Audio))
//     let global = octx
//         .format()
//         .flags()
...
impl Transcoder {
    pub fn new(input_path: PathBuf, output_path: PathBuf) -> Result<Self> {
        ...
        let codec = ffmpeg::encoder::find(output.format().codec(&output_path, media::Type::Audio))
        ...
        // let codec = ffmpeg::encoder::find(codec::id::Id::WAVPACK)
        //     .expect("failed to find MP3 encoder")
        //     .audio()?;
    ...
    pub fn start(&mut self) -> Result<()> {
        self.output.set_metadata(self.input.metadata().to_owned());
        self.output.write_header().unwrap();

        for (stream, mut packet) in self.input.packets() {
    ...
    fn get_and_process_filtered_frames(&mut self) {
            ...
            .is_ok()
        {
            ...
            self.receive_and_process_encoded_packets(); // &self.output);
        }
    }
    ...
    fn receive_and_process_decoded_frames(&mut self) {
        ...
        while self.decoder.receive_frame(&mut decoded).is_ok() {
            ...
            self.get_and_process_filtered_frames(); // ; self.output);
        }
    }
}
