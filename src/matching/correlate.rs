...
impl Correlation {
    ...
    {
        // try to avoid another allocation and compute in place
        let original_size = data.len();
        let mut out = data.to_owned();
        // add padding
        out.append(Axis(0), Array::zeros(size - original_size).view())
            .unwrap();
        let mut out: Array1<Complex<T>> = out
            // data.to_owned()
            // .append(Axis(0), Array::zeros(size - original_size).view())
            // .unwrap()
            .mapv(|v| Complex {
                re: v,
                im: T::zero(),
            });
        // let mut data: Vec<Complex<T>> = data
        // let mut volume_fft: Vec<T> = volume_fft
        ...
        T: FromPrimitive + Signed + Float + FloatConst + std::fmt::Debug + Sync + Send + 'static,
    {
        ...
        // println!("size: {}", size);
        // let mut planner = FftPlanner::new();
        // println!("instantiating took: {:?}", start.elapsed());
        ...
        let volume_fft_handle = thread::spawn(move || {
            Self::padded_fft(&volume, vfft, size2)
            // volume_fft
            //     .append(Axis(0), Array::zeros(size2 - volume_size).view())
            //     .unwrap();
            // let mut volume_fft: Vec<Complex<T>> = volume_fft
            //     // let mut volume_fft: Vec<T> = volume_fft
            //     .into_iter()
            //     .map(|v| Complex {
            //         re: v,
            ...
            //     })
            //     .collect();
            // volume_fft.process(&mut volume_fft);
            // volume_fft
        });
        ...
        let kernel_fft_handle = thread::spawn(move || Self::padded_fft(&kernel, kfft, size2));

        let volume_fft = volume_fft_handle.join().unwrap();
        let kernel_fft = kernel_fft_handle.join().unwrap();

        // kernel
        //     .append(Axis(0), Array::zeros(size2 - kernel_size).view())
        //     .unwrap();
        // let mut kernel_fft: Vec<Complex<T>> = Vec::new();
        // let mut kernel_fft: Vec<Complex<T>> = kernel_fft
        // let mut kernel_fft: Vec<Complex<T>> = kernel_fft
        // let mut kernel_fft: Vec<T> = kernel_fft
        // .into_iter()
        // .map(|v| Complex {
        //     re: v,
        //     im: T::zero(),
        // })
        // .collect();
        // let mut kernel_fft: Vec<Complex<T>> = kernel
        //     .into_iter()
        //     // .into_par_iter()
        //     // .map(|v| chunk.shape().to_owned())
        //     .map(|v| Complex {
        //         re: v,
        //         im: T::zero(),
        //     })
        //     .collect();
        // .collect_into_vec(&mut kernel_fft);

        // println!("data prep: {:?}", start.elapsed());

        // let start = Instant::now();
        // fft.process(&mut kernel_fft);
        // println!("fft took: {:?}", start.elapsed());

        // let volume_fft = Array1::from_iter(volume_fft);
        // let kernel_fft = Array1::from_iter(kernel_fft);
        // println!("volume fft: {:?}", volume_fft.mapv(|v| T::from(v.norm())));
        // println!("kernel fft: {:?}", kernel_fft.mapv(|v| T::from(v.norm())));
        let start = Instant::now();
        let mut ret_fft = (volume_fft * kernel_fft);
        // println!("ret fft: {:?}", ret_fft.mapv(|v| T::from(v.norm())));
        ...
        println!("ifft took: {:?}", start.elapsed());
        // let mut ret_fft = Array1::from(ret_fft);
        // scale the result of the ifft
        // ret_fft.par_mapv_inplace(|v| v.scale(scale));
        // println!("ret ifft: {:?}", ret_fft.mapv(|v| T::from(v.norm())));
        let valid_len = volume_size - kernel_size + 1;
        let valid_start = (size - valid_len) / 2;
        let valid_end = valid_start + valid_len;
        // println!("valid: {} {}..{}", valid_len, valid_start, valid_end);
        ...
        // .par_mapv(|v| T::from(v.norm()).unwrap());
        // println!("ret len: {:?} ({} valid)", ret_fft.len(), valid_len);
        assert!(ret_fft.len() == valid_len);
        ret_fft
    }

    pub fn correlate<T>(volume: &Array1<T>, kernel: &Array1<T>) -> (Array1<T>, (usize, T))
    // pub fn correlate<T>(volume: Arc<Array1<T>>, kernel: Arc<Array1<T>>) -> (Array1<T>, (usize, T))
    where
        T: FromPrimitive + Signed + Float + FloatConst + std::fmt::Debug + Sync + Send + 'static,
    {
        let start = Instant::now();
        let kernel = kernel.slice(s![..;-1]).to_owned();
        // println!("reversed: {:?}", kernel);
        let out = Self::fftconvolve(volume, &kernel);
        // println!("result: {:?}", out);
        println!("fftconvolve took: {:?}", start.elapsed());
        ...
        println!("all took: {:?}", start.elapsed());
        (out, peak)
    }
}
...
#[cfg(test)]
mod tests {
    use crate::matching::correlate::Correlation;
    use ndarray::parallel::prelude::*;
    use ndarray::prelude::*;
    ...
    fn test_correlation() {
        ...
        let (correlation, peak) = Correlation::correlate(&within, &find);
        // let offset = peak.0 / 10;
        // let score = peak.1;
        // println!("offset {:.2} score {:.2?}", offset, score);

        // let correlation = Correlation::correlate(&within, &find);
        assert_eq!(peak.0, 400);
    }
    ...
    fn test_correlation_for_file() {
        ...
        // total.write_all(total_bytes).unwrap();
        // total.seek(SeekFrom::Start(0)).unwrap();
        ...
        // preview.write_all(preview_bytes).unwrap();
        // preview.seek(SeekFrom::Start(0)).unwrap();
        ...
        // within.mapv_inplace(|v| v.abs());
        let within = Zip::from(within.axis_iter(Axis(0)))
            .par_map_collect(|row| row.iter().fold(0f32, |acc, v| acc.max(*v)));
        // let within = Arc::new(within);
        // .map_collect(|row| row.iter().fold(0f32, |acc, v| acc.max(*v)));
