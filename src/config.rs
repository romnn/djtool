use anyhow::Result;
use serde::{de::DeserializeOwned, Deserialize, Serialize};
use serde_json;
use std::path::{Path, PathBuf};
use tokio::io::AsyncReadExt;
// use tokio::io::{BufReader, BufWriter};
use async_trait::async_trait;
use std::marker::{Send, Sync};

#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct Config {
    config_file: PathBuf,
    // spotify_app_client_id
}

#[async_trait]
pub trait Persist: Serialize + DeserializeOwned {
    // Deserialize<'de> {
    // fn config_dir<T: AsRef<Path> + Send + Sync>() -> T;
    // fn config_file_name() -> String;

    // fn config_file<T: AsRef<Path> + Send + Sync>() -> T {
    //     return Self

    async fn load<P: AsRef<Path> + Send + Sync>(config_file: P) -> Result<Self> {
        // let mut file = tokio::fs::File::open(config_dir.as_ref().join("config.json")).await?;
        let mut file = tokio::fs::File::open(config_file).await?;
        // tokio::fs::File::open(config_dir.as_ref().join(Self::config_file_name())).await?;
        let mut buf = String::new();
        file.read_to_string(&mut buf).await?;
        let test = buf.to_owned();
        let deser = serde_json::from_str::<Self>(&test)?;
        Ok(deser)

        // match tokio::fs::File::open(config_dir.as_ref().join("config.json")).await {
        //     Ok(mut file) => {
        //         let mut config = String::new();
        //         file.read_to_string(&mut config).await?;
        //         let config = serde_json::from_str::<Self>(&config)?;
        //         Ok(config)
        //     }
        //     Err(err) => {
        //         println!("err kind: {:?}", err.kind());
        //         if err.kind() == std::io::ErrorKind::NotFound {
        //             Config::new(config_dir).await
        //         } else {
        //             Err(err.into())
        //         }
        //     }
        // }
    }

    async fn save<P: AsRef<Path> + Send + Sync>(&self, config_file: P) -> Result<()> {
        let file = std::fs::OpenOptions::new()
            .write(true)
            .create(true)
            .open(config_file)?;
        // .open(config_dir.as_ref().join(Self::config_file_name()))?;
        // .open(config_dir.as_ref().join("config.json"))?;
        // let file = std::fs::File::open()?;
        let writer = std::io::BufWriter::new(file);
        serde_json::to_writer(writer, &self)?;
        Ok(())
    }
}

impl Config {
    pub async fn new<T: AsRef<Path> + Send + Sync>(config_dir: T) -> Result<Self> {
        let config_file = config_dir.as_ref().join("config.json");
        let mut config = Self {
            // config_dir: config_dir.as_ref().to_owned(),
            config_file: config_file.to_owned(),
        };
        config.save(config_file).await?;
        Ok(config)
    }

    // pub async fn load<T: AsRef<Path>>(config_dir: T) -> Result<Self> {
    //     match tokio::fs::File::open(config_dir.as_ref().join("config.json")).await {
    //         Ok(mut file) => {
    //             let mut config = String::new();
    //             file.read_to_string(&mut config).await?;
    //             let config = serde_json::from_str::<Config>(&config)?;
    //             Ok(config)
    //         }
    //         Err(err) => {
    //             println!("err kind: {:?}", err.kind());
    //             if err.kind() == std::io::ErrorKind::NotFound {
    //                 Config::new(config_dir).await
    //             } else {
    //                 Err(err.into())
    //             }
    //         }
    //     }
    // }

    // pub async fn save<T: AsRef<Path>>(&mut self, config_dir: T) -> Result<()> {
    //     let file = std::fs::OpenOptions::new()
    //         .write(true)
    //         .create(true)
    //         .open(config_dir.as_ref().join("config.json"))?;
    //     // let file = std::fs::File::open()?;
    //     let writer = std::io::BufWriter::new(file);
    //     serde_json::to_writer(writer, &self)?;
    //     Ok(())
    // }
}

impl Persist for Config {
    // fn config_file_name() -> String {
    //     "config.json"
    // }
}
