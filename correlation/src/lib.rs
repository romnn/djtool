    ...
    T: FromPrimitive + Signed + Zero + FftNum + Clone + std::fmt::Debug + Sync + Send + 'static,
{
    ...
    if let Some(fft_out) = out.as_slice_mut() {
        fft.process(fft_out);
        out
    } else {
    ...
    T: FromPrimitive + Signed + Float + FloatConst + std::fmt::Debug + Sync + Send + 'static,
{
    ...
    let inverse_fft = Radix4::new(size2, FftDirection::Inverse);

    let volume_fft = fft.clone();
    let volume_size = volume.len();
    let volume = volume.to_owned();
    let volume_fft_handle = thread::spawn(move || padded_fft(&volume, volume_fft, size2));

    let kernel_fft = fft;
    let kernel_size = kernel.len();
    let kernel = kernel.to_owned();
    let kernel_fft_handle = thread::spawn(move || padded_fft(&kernel, kernel_fft, size2));
    ...
    let ret_fft = if let Some(ret_fft_slice) = ret_fft.as_slice_mut() {
        inverse_fft.process(ret_fft_slice);
        ret_fft
    } else {
        let mut ret_fft = ret_fft.to_vec();
        inverse_fft.process(&mut ret_fft);
        Array1::from(ret_fft)
    };
...
mod tests {
    ...
    #[test]
    fn test_correlation() {
        let within = Array::from_iter(0..501).mapv(|v| f64::from(v));
        let find = Array::from_iter(100..201).mapv(|v| f64::from(v));
        let (_, peak) = correlate(&within, &find);
    ...
    fn test_correlation_for_file() {
        ...
        let start = Instant::now();
        let _within_sample_rate = total_source.sample_rate();
        ...
        within.par_mapv_inplace(f32::abs);
        ...
        let (_correlation, peak) = correlate(&within, &find);
